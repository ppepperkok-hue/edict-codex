...
| M6 | 执行层协议：AGENTS.md 派发循环 + 子 agent 行为契约 | 完成 | v0.6.0 |
...
- ADR-011（M6）：执行层回归原项目异步消息模型——主会话只做队列消费与派发，实际产出由 fork_turns=none 的独立子 agent 完成；子 agent 之间不直接通信，一切经看板与 task_memory。
...
每里程碑按五轴自审并记录审查结论（子 agent 审查为实验特性，当前环境不可用时以自审为准），结论按 Critical / 必须修 / 可选分级，要求附 file:line 证据。
